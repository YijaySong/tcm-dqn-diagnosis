...
def run_prediction(model, env, metadata, symptoms_text, candidate_topk):
    symptoms_list = normalize_symptoms(symptoms_text)
    known = [symptom for symptom in symptoms_list if symptom in env.symptom_to_index]
    unknown = [symptom for symptom in symptoms_list if symptom not in env.symptom_to_index]
    ...
    trace_result = predict_symptoms_trace(
        ...
        min_actions=metadata['config'].get('min_actions_before_stop', 1),
        stop_margin_threshold=metadata['config'].get('stop_margin_threshold'),
    )
...
def build_parser():
    ...
    parser.add_argument('--model-path', default=None, help='V1 best.pt 路径；默认自动选择最新模型')
...
def main():
    ...
    model, env, metadata = load_recommender(args.model_path)
    print(f"加载模型: {metadata['model_path']}")
    print(f"本入口会逐步输出辨证过程，并显示每步候选动作排名前{args.candidate_topk}项。")

    if args.example:
        run_prediction(model, env, metadata, DEFAULT_SYMPTOMS, args.candidate_topk)
        return

    if args.symptoms:
        run_prediction(model, env, metadata, ','.join(args.symptoms), args.candidate_topk)
        return
    ...
    while True:
        ...
        if symptoms_text.lower() == 'q':
            ...
            break
        if symptoms_text:
            run_prediction(model, env, metadata, symptoms_text, args.candidate_topk)
