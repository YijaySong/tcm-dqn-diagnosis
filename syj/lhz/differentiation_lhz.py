...
# lhz_data.txt 数据集格式：id symptom Se
# 其中第二列为 symptom，第三列为 Se。
# 例：1 胸痛,胸痛剧烈,冷痛 寒凝,气滞
def get_tcm_data(filename, max_Se_num=0):
    ...
    with open(filename, 'r', encoding='utf-8') as file:
        header = file.readline().strip()  # 跳过标题行
...
if __name__ == "__main__":
    ...
    logger = createLogger(f"dqn_LR{lr_name}_{nn_units}_{nn_units2}units_{time_sign}.log")
    logger.info(f"可用设备:{device}")
    logger.info(f"随机种子:{args.seed}")

    # 加载 lhz_data.txt，并构建环境。当前脚本位于 syj/lhz，需要回到项目根目录再进入 dataset。
    data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'dataset/lhz_data.txt')
