...
class SupervisedPolicy(object):
    ...
    def update(self, state, action, sess = None):
        sess = sess or tf.get_default_session()
        _, loss, picked_action_prob = sess.run([self.train_op, self.loss, self.picked_action_prob], {self.state: state, self.action: action})
        return loss
    ...
    with tf.Session() as sess:
        ...
        for episode in range(num_samples):
            ...
            for item in good_episodes:
                ...
                loss = policy_nn.update(state_batch, action_batch)
                print(loss)
